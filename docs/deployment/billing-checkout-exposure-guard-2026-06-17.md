...
Memoire now requires an explicit server-side checkout flag before creating a Lemon Squeezy checkout.

This protects the product from accidentally exposing checkout just because Lemon Squeezy API credentials and variant IDs are present in the deployment environment.

Updated 2026-07-31: the payment provider moved from Stripe to Lemon Squeezy. The flag, the failure modes and this rule are unchanged - only the provider behind them is different.
...
- `/api/billing` still fails closed when `LEMONSQUEEZY_API_KEY` or `LEMONSQUEEZY_STORE_ID` is missing.
- Checkout now also requires `BILLING_CHECKOUT_ENABLED=true`.
- If Lemon Squeezy is configured but the checkout flag is not true, checkout returns `503` with `Checkout is not enabled.`
- Portal access remains separate for future paid users who already have a Lemon Squeezy subscription or customer.
...
- B5: pricing can remain hypothesis-mode while backend Lemon Squeezy config is prepared in preview.
...
- Billing QA includes a case where Lemon Squeezy is configured but checkout flag is false.
...
- In protected preview, configure Lemon Squeezy test-mode credentials and variant IDs while `BILLING_CHECKOUT_ENABLED=false`; a signed-in checkout attempt must return `503` and create no Lemon Squeezy checkout.
