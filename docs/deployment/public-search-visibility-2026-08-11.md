...
3. ~~**The Open Graph card is still an SVG.**~~ Done 2026-08-26. The estimate
   ...
   One footnote worth keeping: deleting `public/social-card.svg` did **not**
   retire that URL. The catch-all rewrite in `vercel.json` answered it with the
   HTML shell and a 200 - the same trap `/favicon.ico` fell into before it
   existed, a successful response of type `text/html` to a request for an
   image. LinkedIn and Facebook cache an `og:image` URL for weeks and a 200
   does not invalidate that cache, so `vercel.json` now carries a permanent
   redirect from the old card to the new one. Any public asset URL that is
   removed needs the same treatment; there is no 404 on this deployment.
