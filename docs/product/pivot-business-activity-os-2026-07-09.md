...
- **Phase 1 (this change):** strategy docs, Activity Ledger with domain filters, expanded activity taxonomy + capture templates (payment, delivery, partnership, content, product, learning), nav + brand label, landing repositioning, demo data for non-sales activities.
- **Phase 2 (next):** Weekly Brief -> Weekly Business Review (money flow, stalled initiatives, wins/losses, next-week priorities; Pipeline Defense Brief embedded as the manager-facing artifact). Today reframed as the business cockpit answering: what moves money today / which deals are hot / which follow-ups are late / which initiative is stuck / what must be captured. Initiative-stalled and money-stuck nudges in the existing nudge engine.
- **Phase 3 (cohort-gated):** unified money-flow thread (deal -> quote -> PO -> delivery -> payment) as one lifecycle view; offers/experiments as first-class initiative types; Ask Memoire business-state questions ("where is the money?", "what happened this week?").
