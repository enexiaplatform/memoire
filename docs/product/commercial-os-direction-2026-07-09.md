...
| 7.6 Initiatives & experiments | SHIPPED (2026-07-11) | hypothesis/expected signal/current signal/decision on `payload`; related-activity read-model on the operating page. Ask Memoire now answers "how are my experiments going / which initiative should I revisit" from the shared `initiativeReview` read-model (measured health + experiment signal, honest empty state, no AI) - the Learning end of the loop is queryable |
